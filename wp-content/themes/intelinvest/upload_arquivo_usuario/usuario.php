...
class Usuario {
	...
	public function criarNovaPasta($caminho, $pasta){
	...
	public function moverArquivo($arquivo, $caminho, $novoCaminho ){
		rename($caminho."/".$arquivo, $novoCaminho."/".$arquivo);

	}

	public function excluirArquivo($caminho, $idUser, $arquivo){
		if (file_exists($caminho ."/". $idUser . "/" . $arquivo) and !empty($arquivo)){
			// Removendo arquivo
	        unlink($caminho . "/". $idUser . "/" . $arquivo);
	        return true;
		}else{
			return false;
		}
